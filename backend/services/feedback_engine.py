...
def analyze_issues(
    ...
    jd_comparison: Optional[Dict] = None,
) -> List[IssueDetail]:

    jd_match = jd_comparison or {}
    jd_skills_gap = jd_match.get("skills_gap", [])
    resume_projects = projects or []
    project_text = " ".join(
    ...
    for skill in jd_skills_gap[:5]:
        if skill.lower() not in project_text.lower():
            jd_gap_recommendations.append(
                f"Build a project feature using {skill} before adding it to your resume."
            )

    missing_jd_keywords = jd_match.get("missing_keywords", [])
    ...
    if total_skills > 0 and len(unvalidated) > len(validated):
        ...
        action_items_skills = [
            f"Add '{skill}' to a project or experience bullet only if you genuinely used it there."
            for skill in unvalidated[:5]
        ]
        action_items_skills.append(
            "Remove a skill if you cannot genuinely demonstrate where you used it."
        )
        ...
        detected.append(IssueDetail(
            ...
            example_improvement=(
                ...
                f"Fix: If you genuinely used {unvalidated[0]} in a project or experience, "
                "describe the actual work you performed. Otherwise, build relevant "
                "experience with the skill before adding it to your resume."
            ),
        ))

        # Rule 5B: JD-specific skill gaps
        if jd_comparison and jd_skills_gap:
            gap_list = ", ".join(jd_skills_gap[:8])

            action_items_jd = jd_gap_recommendations

            action_items_jd.append(
                "Only add a missing skill to your resume after you can genuinely demonstrate it through a project or experience."
            )


            detected.append(IssueDetail(
                issue_title="Important Skills Missing for This Job",
                severity_level="High",
                ats_impact="High",
                explanation=(
                    f"The job description expects skills that are not currently "
                    f"demonstrated on your resume: {gap_list}."
                ),
                where_it_appears=f"JD-specific skill gaps: {gap_list}",
                how_to_fix=(
                    "Only add a missing skill after you can genuinely demonstrate it "
                    "through a project or experience."
                ),
                action_items=action_items_jd,
                example_improvement=(
                    "Build or update a relevant project using the missing skill, "
                    "then describe the actual work performed in your resume."
                ),
            ))
